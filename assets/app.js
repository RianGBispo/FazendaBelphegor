...
const SHEET_URL = 'https://docs.google.com/spreadsheets/d/' + CONFIG.SHEET_ID + '/edit';
...
const TRUES = ['true', 'sim', 's', 'yes', 'y', 'pago', 'x', '1', '✔', '✓', '✅', 'ok'];
function toBool(raw) {
...
function toDate(raw) {
  if (raw === null || raw === undefined || raw === '') return { label: '—', ts: 0 };
...
function mapColumns(cols) {
  const map = {};
  const labels = cols.map((c, i) => normalizeLabel(c.label) || String.fromCharCode(65 + i));
  for (const [key, aliases] of Object.entries(COLUMNS)) {
    ...
    if (idx === -1) idx = labels.findIndex((l) => wanted.some((w) => l.startsWith(w)));
    map[key] = idx;
  }
  // fallback posicional (ordem A:K da planilha) para colunas não encontradas
  const order = ['discordId', 'nome', 'tipo', 'quantidade', 'total', 'data',
    'aprovadoPor', 'pago', 'cargo', 'pagamento', 'pct'];
  order.forEach((key, i) => { if (map[key] === -1 && cols[i]) map[key] = i; });
  return map;
}

async function fetchSheet() {
  const url = 'https://docs.google.com/spreadsheets/d/' + CONFIG.SHEET_ID +
    '/gviz/tq?tqx=out:json&headers=1&sheet=' + encodeURIComponent(CONFIG.SHEET_NAME) +
    '&_=' + Date.now();
  ...
  const records = (table.rows || []).map((row, i) => {
    ...
    const pagamento = (rawPag === null || rawPag === '') ? total * pct : toNum(rawPag);
  ...
  records.forEach((r) => { if (!r.discordId) r.discordId = 'nome:' + r.nome.toLowerCase(); });
  return records;
}
...
function demoRecords() {
  const PRECO = { Trigo: 120, Milho: 95, Leite: 210, Ovos: 65, Uva: 180, 'Lã': 150 };
  const CARGOS = { 'Líder': 0.35, 'Vice-Líder': 0.3, 'Gerente': 0.25, 'Farmeiro': 0.15, 'Novato': 0.1 };
  const MEMBROS = {
  ...
  return RAW.map((r, i) => {
    ...
    const d = toDate(r[3]);
    return {
      key: i, discordId: id, nome: r[0], tipo: r[1], qtd: r[2], total,
      data: d.label, ts: d.ts, aprovadoPor: r[4], pago: r[5],
      cargo, pagamento: total * pct, pct
    };
  });
}
...
function aggregates() {
  ...
  for (const r of state.records) {
    const a = byId[r.discordId] || (byId[r.discordId] = {
      nome: r.nome, discordId: r.discordId, cargo: r.cargo, pct: r.pct,
      total: 0, qtd: 0, pagamento: 0, pago: 0, pendente: 0, count: 0
    });
    a.total += r.total;
    a.qtd += r.qtd;
    a.pagamento += r.pagamento;
    a.count += 1;
    if (r.pago) a.pago += r.pagamento; else a.pendente += r.pagamento;
...
const tipos = () => [...new Set(state.records.map((r) => r.tipo))].sort((a, b) => a.localeCompare(b, 'pt-BR'));
...
function goto(view, membroId) {
  const hash = view === 'membro' && membroId ? `#/membro/${encodeURIComponent(membroId)}` : `#/${view}`;
...
function applyRoute() {
  ...
  state.view = view;
  if (view === 'membro' && parts[1]) state.membroId = decodeURIComponent(parts[1]);
  render();
}
...
function render() {
  ...
  if (state.loading) { $view.dataset.view = 'loading'; $view.innerHTML = skeleton(); return; }
  if (state.error) { $view.dataset.view = 'error'; $view.innerHTML = ''; return; }
  if (!state.records.length) { $view.dataset.view = 'empty'; $view.innerHTML = emptyState(); return; }

  if (state.view === 'registros' && $view.dataset.view === 'registros') { updateRegistros(); return; }

  $view.dataset.view = state.view;
  if (state.view === 'dash') $view.innerHTML = viewDash();
  else if (state.view === 'registros') { $view.innerHTML = viewRegistros(); updateRegistros(); }
  else if (state.view === 'ranking') $view.innerHTML = viewRanking();
  else $view.innerHTML = viewMembro();
}

const skeleton = () => `<div class="grid-kpi">${'<div class="skeleton"></div>'.repeat(4)}</div>
...
function viewDash() {
  ...
  const pagoGeral = recs.filter((r) => r.pago).reduce((s, r) => s + r.pagamento, 0);
  const pendenteGeral = recs.filter((r) => !r.pago).reduce((s, r) => s + r.pagamento, 0);
  const t = list[0];

  const kpis = [
    ['Total farmado', money(totalGeral), recs.length + ' registros na planilha'],
    ['Já pago', money(pagoGeral), 'confirmado pela gerência'],
    ['A pagar', money(pendenteGeral), recs.filter((r) => !r.pago).length + ' registros pendentes'],
    ['Membros ativos', String(list.length), 'com registro no período']
  ];

  const pendentes = list.filter((m) => m.pendente > 0).sort((a, b) => b.pendente - a.pendente);
  const recentes = recs.slice().sort((a, b) => b.ts - a.ts || b.key - a.key).slice(0, 6);
  const chasers = list.slice(1, 5);

  return `<section class="section">
    <div class="grid-kpi">
      ${kpis.map(([l, v, n]) => `<div class="kpi">
    ...
    <div class="grid-2">
      ...
      <div class="card">
        <div class="card-title">Perseguidores</div>
        <div class="chasers">
          ${chasers.length ? chasers.map((m, i) => `<button class="chaser" data-membro="${esc(m.discordId)}">
    ...
    <div class="grid-2-sm">
      <div class="card">
        ...
        <div class="list">
          ${pendentes.length ? pendentes.map((m) => `<div class="row-line row-pend">
      ...
      <div class="card">
        ...
        <div class="list">
          ${recentes.map((r) => `<div class="row-line row-rec">
      ...
      <div class="filters-right">
        ...
        <div class="segment" id="statusSeg">
          ${['Todos', 'Pago', 'Pendente'].map((s) =>
            `<button data-status="${s}" class="${state.status === s ? 'on' : ''}">${s}</button>`).join('')}
        </div>
      </div>
    </div>
...
function updateRegistros() {
  ...
  document.getElementById('regBody').innerHTML = rows.length ? rows.map((r) => `<tr>
...
function viewRanking() {
  ...
  return `<section class="ranking">
    ${list.map((m, i) => `<button class="rank-card${i === 0 ? ' first' : ''}" data-membro="${esc(m.discordId)}">
  ...
  return `<section class="section">
    ...
    <div class="member-hero">
      ...
      <div class="member-kpis">
        ${kpis.map(([l, v]) => `<div class="member-kpi">
            <div class="kpi-label">${esc(l)}</div>
            <div class="kpi-value">${esc(v)}</div>
          </div>`).join('')}
      </div>
...
$view.addEventListener('click', (e) => {
  ...
  if (btn) { goto('membro', btn.dataset.membro); return; }
...
$view.addEventListener('input', (e) => {
  if (e.target.id === 'busca') { state.busca = e.target.value; updateRegistros(); }
});

$view.addEventListener('change', (e) => {
  if (e.target.id === 'tipoSel') { state.tipo = e.target.value; updateRegistros(); }
});
...
function stamp() {
  ...
  document.getElementById('sheetLabel').textContent =
    CONFIG.SHEET_LABEL + ' · atualizado ' + pad2(d.getHours()) + ':' + pad2(d.getMinutes());
}
...
async function load(manual) {
  ...
  if (new URLSearchParams(location.search).has('demo')) {
    ...
    state.loading = false;
    state.error = null;
    setStatus('Modo demonstração — dados de exemplo, não vindos da planilha. Remova <code>?demo=1</code> da URL para usar os dados reais.');
    stamp();
    render();
    btn.disabled = false;
  ...
  try {
    ...
    state.error = err;
    setStatus(`<strong>Não consegui ler a planilha.</strong> ${esc(err.message)}<br>
      Confira se ela está compartilhada como <em>“Qualquer pessoa com o link — Leitor”</em>
      e se a aba se chama <code>${esc(CONFIG.SHEET_NAME)}</code>.`, true);
    document.getElementById('sheetLabel').textContent = CONFIG.SHEET_LABEL + ' · falha ao ler';
  } finally {
    state.loading = false;
    btn.disabled = false;
    btn.textContent = 'Atualizar';
...
if (CONFIG.AUTO_REFRESH_MS > 0) {
  setInterval(() => { if (!document.hidden) load(); }, CONFIG.AUTO_REFRESH_MS);
}
